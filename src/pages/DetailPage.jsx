...
import { ArrowLeft, Edit3, Trash2, Save, Package, Calendar, Hash, FileText, ShoppingBag } from 'lucide-react';
...
export function DetailPage({ productId, onBack }) {
  ...
  const { status, label, daysLeft } = getExpiryStatus(product.expiryDate, settings.warningDays);
          ...
          <InfoRow icon={<Calendar size={16} />} label="Hạn sử dụng">
            {editing ? (
              <input
                ...
                className="text-white text-sm bg-transparent outline-none text-right"
